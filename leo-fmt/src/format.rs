...
pub fn format_node(node: &SyntaxNode, out: &mut Output) {
    match &node.kind {
        ...
        SyntaxKind::Expression(ExpressionKind::Struct) => format_struct_expr(node, out),
        ...
        SyntaxKind::Expression(ExpressionKind::Locator) => format_locator(node, out),
        ...
        SyntaxKind::StructMemberInitializer => format_struct_member_init(node, out),
...
fn format_struct_member_list(node: &SyntaxNode, out: &mut Output) {
    let members: Vec<_> =
        node.children.iter().filter(|c| matches!(c.kind, SyntaxKind::StructMemberDeclaration)).collect();
    ...
    for member in &members {
        out.indented(|out| {
            format_struct_member(member, out);
            out.write(",");
        });
        out.newline();
    }
...
fn format_struct_expr(node: &SyntaxNode, out: &mut Output) {
    let inits: Vec<_> =
        node.children.iter().filter(|c| matches!(c.kind, SyntaxKind::StructMemberInitializer)).collect();
    ...
    for child in &node.children {
        match &child.kind {
            SyntaxKind::Token if child.text == "{" => {
                ...
                if !inits.is_empty() {
                    ...
                    for (i, init) in inits.iter().enumerate() {
                        format_struct_member_init(init, out);
                        if i < inits.len() - 1 {
                            out.write(",");
                            out.space();
                        }
            ...
            SyntaxKind::Token => out.write(child.text),
            SyntaxKind::ConstArgumentList => format_const_argument_list(child, out),
            SyntaxKind::StructMemberInitializer => {} // handled above
            _ => {}
        }
    }
}

fn format_struct_member_init(node: &SyntaxNode, out: &mut Output) {
...
fn format_locator(node: &SyntaxNode, out: &mut Output) {
    for child in &node.children {
        if let SyntaxKind::Token = &child.kind {
            out.write(child.text);
        }
