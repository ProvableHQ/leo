...
impl<'a> ImportParser<'a> {
    ...
    pub(crate) fn parse_package(
        ...
        let entries = fs::read_dir(path)
            .map_err(|error| ImportError::directory_error(error, &error_path, span, new_backtrace()))?
        ...
        if imports_directory.exists() {
            ...
            match (matched_source_entry, matched_import_entry) {
                (Some(_), Some(_)) => Err(ImportError::conflicting_imports(package_name, span, new_backtrace()).into()),
                ...
                (None, None) => Err(ImportError::unknown_package(package_name, span, new_backtrace()).into()),
            }
        } else {
            ...
            match matched_source_entry {
                ...
                None => Err(ImportError::unknown_package(package_name, span, new_backtrace()).into()),
            }
        }
    }
}
