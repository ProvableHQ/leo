...
impl InputDirectory {
    ...
    pub fn create(path: &Path) -> Result<()> {
        ...
        if path.is_dir() && !path.ends_with(INPUT_DIRECTORY_NAME) {
            path.to_mut().push(INPUT_DIRECTORY_NAME);
        }

        fs::create_dir_all(&path).map_err(PackageError::failed_to_create_input_directory)?;
        Ok(())
    }
    ...
    pub fn files(path: &Path) -> Result<Vec<PathBuf>> {
        ...
        path.push(INPUT_DIRECTORY_NAME);

        let directory = fs::read_dir(&path).map_err(PackageError::failed_to_read_input_directory)?;
...
fn parse_file_paths(directory: ReadDir, file_paths: &mut Vec<PathBuf>) -> Result<()> {
    for file_entry in directory {
        ...
        if file_type.is_dir() {
            let directory = fs::read_dir(&file_path).map_err(PackageError::failed_to_read_input_directory)?;
