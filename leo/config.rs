...
impl Config {
    ...
    pub fn read_config() -> Result<Self> {
        ...
        if !Path::exists(&config_path) {
            ...
            create_dir_all(&config_dir).map_err(|e| CliError::cli_io_error(e, new_backtrace()))?;

            let default_config_string = toml::to_string(&Config::default())
                .map_err(|e| CliError::failed_to_convert_to_toml(e, new_backtrace()))?;

            fs::write(&config_path, default_config_string).map_err(|e| CliError::cli_io_error(e, new_backtrace()))?;
        }
        ...
        let config: Config =
            toml::from_str(&toml_string).map_err(|e| CliError::failed_to_convert_from_toml(e, new_backtrace()))?;
    ...
    pub fn set_update_automatic(automatic: bool) -> Result<()> {
        ...
        if config.update.automatic != automatic {
            ...
            fs::write(
                &config_path,
                toml::to_string(&config).map_err(|e| CliError::failed_to_convert_to_toml(e, new_backtrace()))?,
            )
            .map_err(|e| CliError::cli_io_error(e, new_backtrace()))?;
        }
...
pub fn write_token_and_username(token: &str, username: &str) -> Result<()> {
    ...
    let mut credentials =
        File::create(&LEO_CREDENTIALS_PATH.to_path_buf()).map_err(|e| CliError::cli_io_error(e, new_backtrace()))?;
    credentials
        .write_all(token.as_bytes())
        .map_err(|e| CliError::cli_io_error(e, new_backtrace()))?;

    let mut username_file =
        File::create(&LEO_USERNAME_PATH.to_path_buf()).map_err(|e| CliError::cli_io_error(e, new_backtrace()))?;
    username_file
...
pub fn read_token() -> Result<String> {
    let mut credentials =
        File::open(&LEO_CREDENTIALS_PATH.to_path_buf()).map_err(|e| CliError::cli_io_error(e, new_backtrace()))?;
...
pub fn read_username() -> Result<String> {
    let mut username =
        File::open(&LEO_USERNAME_PATH.to_path_buf()).map_err(|e| CliError::cli_io_error(e, new_backtrace()))?;
    let mut buf = String::new();
    username
        .read_to_string(&mut buf)
        .map_err(|e| CliError::cli_io_error(e, new_backtrace()))?;
