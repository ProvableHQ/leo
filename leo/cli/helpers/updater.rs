...
const LEO_CACHE_LAST_CHECK_FILE: &str = "leo_cache_last_update_check";
const LEO_CACHE_VERSION_FILE: &str = "leo_cache_latest_version";
const LEO_REPO_NAME: &str = "leo";
const LEO_REPO_OWNER: &str = "ProvableHQ";
// 24 hours
...
pub fn read_latest_version() -> Result<Option<String>, CliError> {
    let version_file_path = get_version_file_path();
...
pub fn get_cli_string() -> Result<Option<String>, CliError> {
    if let Some(latest_version) = read_latest_version()? {
...
pub fn print_cli() -> Result<(), CliError> {
    if let Some(message) = get_cli_string()? {
        println!("{}", message);
    }
    Ok(())
}
...
pub fn check_for_updates(force: bool) -> Result<bool, CliError> {
    ...
    let last_check_file = cache_dir.join(LEO_CACHE_LAST_CHECK_FILE);
    let version_file = get_version_file_path();

    // Determine if we should check for updates.
    let should_check = force || should_check_for_updates(&last_check_file)?;

    if should_check {
        match update_available() {
            Ok(latest_version) => {
                // A new version is available
                update_check_files(&cache_dir, &last_check_file, &version_file, &latest_version)?;
                Ok(true)
            }
            Err(_) => {
                // No new version available or error occurred
                // We'll treat both cases as "no update" for simplicity
                update_check_files(&cache_dir, &last_check_file, &version_file, env!("CARGO_PKG_VERSION"))?;
                Ok(false)
            }
        }
...
fn should_check_for_updates(last_check_file: &Path) -> Result<bool, CliError> {
    match fs::read_to_string(last_check_file) {
        Ok(contents) => {
            ...
            Ok(current_time.saturating_sub(last_check) > LEO_UPDATE_CHECK_INTERVAL.as_secs())
        }
...
fn get_version_file_path() -> PathBuf {
    get_cache_dir().join(LEO_CACHE_VERSION_FILE)
}
