...
impl<F: Field + PrimeField, G: GroupType<F>> ConstrainedProgram<F, G> {
    ...
    pub fn enforce_iteration_statement<CS: ConstraintSystem<F>>(
        ...
        let from = self.enforce_index(cs, file_scope, function_scope, &statement.start, &span)?;
        let to = self.enforce_index(cs, file_scope, function_scope, &statement.stop, &span)?;
