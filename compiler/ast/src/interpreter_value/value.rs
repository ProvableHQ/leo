...
impl Value {
    ...
    pub fn to_svm_compatible(&self) -> Option<Self> {
        match &self.contents {
            // Recursively process tuple elements
            ValueVariants::Tuple(elements) => {
                let mut converted = Vec::with_capacity(elements.len());
                for elem in elements {
                    let svm_value = elem.to_svm_compatible()?; // recursive call
                    converted.push(svm_value);
                }

                Some(Value { id: self.id.clone(), contents: ValueVariants::Tuple(converted) })
            }

            // For all other cases, try converting to SvmValue directly
            _ => {
                let svm_value = SvmValue::try_from(self.clone()).ok()?;
                Some(Value { id: self.id.clone(), contents: ValueVariants::Svm(svm_value) })
            }
        }
    }
