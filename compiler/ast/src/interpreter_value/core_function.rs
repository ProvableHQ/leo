...
use super::{Address, Boolean, Field, Group, LeoValue, Plaintext, PlaintextHash, Scalar};
...
pub trait CoreFunctionHelper {
    ...
    fn lookup_mapping(&self, _program: Option<Symbol>, _name: Symbol) -> Option<&HashMap<PlaintextHash, Plaintext>> {
        None
    }
...
impl CoreFunctionHelper for Vec<LeoValue> {
    fn pop_value_impl(&mut self) -> Option<LeoValue> {
        self.pop()
    }
}
...
pub fn evaluate_core_function(
    ...
    span: Span,
) -> Result<Option<LeoValue>> {
    use snarkvm::{
        prelude::LiteralType,
        synthesizer::program::{CommitVariant, HashVariant},
    };

    let dohash = |helper: &mut dyn CoreFunctionHelper, variant: HashVariant, typ: LiteralType| -> Result<LeoValue> {
    ...
    let docommit =
        |helper: &mut dyn CoreFunctionHelper, variant: CommitVariant, typ: LiteralType| -> Result<LeoValue> {
            let randomizer: Scalar = helper.pop_value()?.try_into().expect_tc(span)?;
            let input: Value = helper.pop_value()?.try_into().expect_tc(span)?;
    ...
    let value = match core_function {
        ...
        CoreFunction::MappingGet => {
            let key: Plaintext = helper.pop_value()?.try_into().expect_tc(span)?;
            let (program, name) = match &arguments[0] {
                Expression::Identifier(id) => (None, id.name),
                Expression::Locator(locator) => (Some(locator.program.name.name), locator.name),
                _ => tc_fail!(),
            };
            match helper.lookup_mapping(program, name).and_then(|mapping| mapping.get(&PlaintextHash(key))) {
                Some(v) => v.clone().into(),
        ...
        CoreFunction::MappingSet => {
            let value: Plaintext = helper.pop_value()?.try_into().expect_tc(span)?;
            let key: Plaintext = helper.pop_value()?.try_into().expect_tc(span)?;
            let (program, name) = match &arguments[0] {
                Expression::Identifier(id) => (None, id.name),
                Expression::Locator(locator) => (Some(locator.program.name.name), locator.name),
            ...
            if let Some(mapping) = helper.lookup_mapping_mut(program, name) {
                mapping.insert(PlaintextHash(key), value);
            } else {
                tc_fail!();
            }
            LeoValue::Unit
        ...
        CoreFunction::MappingRemove => {
            ...
            if let Some(mapping) = helper.lookup_mapping_mut(program, name) {
                mapping.remove(&PlaintextHash(key));
            } else {
                tc_fail!();
            }
            LeoValue::Unit
        ...
        CoreFunction::MappingContains => {
            ...
            if let Some(mapping) = helper.lookup_mapping_mut(program, name) {
                mapping.contains_key(&PlaintextHash(key)).into()
            } else {
                tc_fail!();
            }
        }
