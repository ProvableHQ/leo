...
impl<'t, 's> Parser<'t, 's> {
    ...
    /// Look ahead by `n` tokens and return the kind.
    ...
    ///
    /// This is useful for operators where we need to wrap the left-hand side
    /// after seeing the operator.
    ...
    ///
    /// This wraps everything from the checkpoint to now in a new node.
