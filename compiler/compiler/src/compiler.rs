...
impl Compiler {
    ...
    pub fn compile_from_file(&mut self, source_file_path: impl AsRef<Path>) -> Result<String> {
        ...
        // Determine the root directory by getting the parent of the main file.
        let root_dir =
            source_file_path.as_ref().parent().expect("Expected file to have a parent directory").to_path_buf();

        // Walk all files under root_dir recursively, excluding the main source file itself.
        let mut files = WalkDir::new(root_dir.clone())
        .into_iter()
        .filter_map(Result::ok) // Skip entries that result in errors.
        .filter(|e| e.file_type().is_file() && e.path() != source_file_path.as_ref())
        .collect::<Vec<_>>();

        // Sort files by depth in descending order so deeper (nested) modules are compiled first.
        // We do this for now because we only have relative paths and sorting by depth is sufficient to ensure
        // we process modules in the right order.
        //
        // In the future, we want to do this semantic checking where we would have to analyze all the paths
        // (including absolute paths when they're implemented) and figure out a topological sort that works.
        files.sort_by(|a, b| {
            ...
            depth_b.cmp(&depth_a) // deeper files first
        });
