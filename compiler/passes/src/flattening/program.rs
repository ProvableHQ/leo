...
use leo_ast::{
    AstReconstructor, Constructor, Expression, Function, ProgramReconstructor, ProgramScope, ReturnStatement, Statement,
};
        ...
        let expression_returns: Vec<(Option<Expression>, ReturnStatement)> = returns
            ...
            .map(|(guard, statement)| match guard {
                ...
                ReturnGuard::Unconstructed(plain) | ReturnGuard::Constructed { plain, .. } => {
                    (Some(leo_ast::Path::from(plain).into()), statement)
                }
            })
            .collect();
