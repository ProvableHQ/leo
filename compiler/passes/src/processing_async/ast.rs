            ...
            |slf: &mut Self, symbol: Symbol, var_type: &Type, index_opt: Option<usize>| -> Vec<(Input, Expression)> {
                ...
                match index_opt {
                    Some(index) => {
                        ...
                        replacements.insert((symbol, Some(index)), Path::from(identifier).with_absolute_path(Some(vec![identifier.name])).into());

                        vec![(
                            input,
                            TupleAccess {
                                tuple: Path::from(make_identifier(slf, symbol)).with_absolute_path(Some(vec![make_identifier(slf, symbol).name])).into(),
                                index: index.into(),
                                span: Span::default(),
                                id: slf.state.node_builder.next_id(),
                            }
                    ...
                    None => match var_type {
                        Type::Tuple(TupleType { elements }) => {
                            ...
                            for (i, element_type) in elements.iter().enumerate() {
                                ...
                                let expr: Expression = Path::from(identifier).with_absolute_path(Some(vec![identifier.name])).into();
                        ...
                        _ => {
                            ...
                            replacements.insert((symbol, None), Path::from(identifier).with_absolute_path(Some(vec![identifier.name])).into());

                            let argument = Path::from(make_identifier(slf, symbol)).with_absolute_path(Some(vec![make_identifier(slf, symbol).name])).into();
                            vec![(input, argument)]
                        }
                    },
                }
