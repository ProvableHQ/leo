...
impl TypeCheckingVisitor<'_> {
    pub fn visit_expression_assign(&mut self, input: &Expression) -> Type {
        let ty = match input {
            ...
            Expression::Path(path) if path.symbols().len() == 1 => self.visit_path_assign(path),
        ...
        if external_record || external_record_tuple {
            ...
            if !self.symbol_in_conditional_scope(path.as_symbol()) {
                if external_record {
    ...
    pub fn visit_member_access_general(&mut self, input: &MemberAccess, assign: bool, expected: &Option<Type>) -> Type {
        ...
        if !assign {
            match &input.inner {
                ...
                Expression::Path(path) if path.as_symbol() == sym::SelfLower => {
                ...
                Expression::Path(path) if path.as_symbol() == sym::block => match input.name.name {
                    sym::height => {
                ...
                Expression::Path(path) if path.as_symbol() == sym::network => match input.name.name {
                    sym::id => {
    ...
    pub fn visit_path_assign(&mut self, input: &Path) -> Type {
        ...
        if self.scope_state.variant.unwrap().is_async_function() && !self.symbol_in_conditional_scope(input.as_symbol())
        {
        ...
        if self.async_block_id.is_some() && !self.symbol_in_conditional_scope(input.as_symbol()) {
        ...
        if let Some(async_block_id) = self.async_block_id {
            if !self.state.symbol_table.is_defined_in_scope_or_ancestor_until(async_block_id, input.as_symbol()) {
        ...
        for (expected, argument) in func.input.iter().zip(input.arguments.iter()) {
            ...
            if func.variant == Variant::AsyncFunction && matches!(expected.type_(), Type::Future(_)) {
                ...
                let option_name = match argument {
                    Expression::Path(path) => Some(path.as_symbol()),
