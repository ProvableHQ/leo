        ...
        if !(input.is_record && self.scope_state.is_stub) {
            for Member { mode, identifier, type_, span, .. } in input.members.iter() {
                ...
                if let Type::Composite(composite_member_type) = type_ {
                    // Note that since there are no cycles in the program dependency graph, there are no cycles in the composite dependency graph caused by external composites.
                    self.state.composite_graph.add_edge(
                        Location::new(this_program, composite_path),
                        Location::new(
                            composite_member_type.program.unwrap_or(this_program),
                            composite_member_type.path.absolute_path().to_vec(),
                        ),
                    );
                    ...
                    if let Type::Composite(member_type) = base_element_type {
                        self.state.composite_graph.add_edge(
                            Location::new(this_program, composite_path),
                            Location::new(
                                member_type.program.unwrap_or(this_program),
                                member_type.path.absolute_path().to_vec(),
                            ),
                        );
                    }
                }
