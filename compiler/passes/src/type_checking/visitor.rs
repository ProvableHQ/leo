...
impl TypeCheckingVisitor<'_> {
        ...
        let arguments = match core_function {
            ...
            CoreFunction::OptionalUnwrapOr => {
                ...
                if let Some(expected) = expected {
                    ...
                    let Type::Optional(OptionalType { ref inner }) = opt_ty else {
                        return Type::Err;
                    };
                    let fallback_ty = self.visit_expression(fallback, &Some(*inner.clone()));
                    vec![(opt_ty, opt), (fallback_ty, fallback)]
                }
            }
            CoreFunction::Get => {
            ...
            CoreFunction::VectorPush => {
                ...
                let Type::Vector(VectorType { ref element_type }) = vec_ty else {
                    return Type::Err;
                };
                let val_ty = self.visit_expression(val, &Some(*element_type.clone()));
            ...
            _ => {
                // Get the types of the arguments. Error out on arguments that have `Type::Numeric`.
                // TODO: We could potentially do better for some core functions.
    ...
    fn contains_optional_type_inner(&mut self, ty: &Type, visited_paths: &mut IndexSet<Vec<Symbol>>) -> bool {
        match ty {
            ...
            Type::Vector(vector) => self.contains_optional_type_inner(&vector.element_type, visited_paths),
    ...
    pub fn check_function_signature(&mut self, function: &Function, is_stub: bool) {
        ...
        for (i, input) in function.input.iter().enumerate() {
            ...
            if matches!(
                self.scope_state.variant,
                Some(Variant::AsyncTransition) | Some(Variant::Transition) | Some(Variant::Function)
            ) && self.contains_optional_type(&input.type_)
            {
                self.emit_err(TypeCheckerError::input_cannot_be_optional(
                    self.scope_state.variant.unwrap(),
                    function.identifier,
                    input.span,
                ));
            }
        ...
        function.output.iter().enumerate().for_each(|(index, function_output)| {
            ...
            if matches!(
                self.scope_state.variant,
                Some(Variant::AsyncTransition) | Some(Variant::Transition) | Some(Variant::Function)
            ) && self.contains_optional_type(&function_output.type_)
            {
                self.emit_err(TypeCheckerError::output_cannot_be_optional(
                    self.scope_state.variant.unwrap(),
                    function.identifier,
