...
impl TypeCheckingVisitor<'_> {
    ...
    pub fn contains_optional_type(&mut self, ty: &Type) -> bool {
        match ty {
            ...
            Type::Tuple(tuple) => tuple.elements.iter().any(|e| self.contains_optional_type(e)),
            Type::Array(array) => self.contains_optional_type(&array.element_type),
            Type::Composite(struct_type) => {
                if let Some(comp) = self.lookup_struct(
                    struct_type.program.or(self.scope_state.program_name),
                    &struct_type.path.absolute_path(),
                ) {
                    comp.members.iter().any(|Member { type_, .. }| self.contains_optional_type(type_))
                } else {
                    false
                }
            }
