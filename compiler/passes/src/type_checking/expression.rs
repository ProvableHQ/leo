...
impl ExpressionVisitor for TypeCheckingVisitor<'_> {
    ...
    fn visit_call(&mut self, input: &CallExpression, expected: &Self::AdditionalInput) -> Self::Output {
        ...
        for (expected, argument) in func.input.iter().zip(input.arguments.iter()) {
            ...
            let ty = match expected.type_() {
                Type::Array(array_type) if array_type.length().is_none() => {
                    self.visit_expression_reject_numeric(argument, &None)
                }
                _ => self.visit_expression_reject_numeric(argument, &Some(expected.type_().clone())),
            };
