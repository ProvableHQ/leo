    ...
    fn reconstruct_identifier(&mut self, input: Identifier) -> (Expression, Vec<Statement>) {
        ...
        if let Some(array_members) = self.array_members.get(&input.name) {
            ...
            let statement = AssignStatement {
                place: Path::from(input).with_local(input.name).into(),
                value: expr.into(),
                span: Default::default(),
                id: self.state.node_builder.next_id(),
            };
            statements.push(statement.into());
            (Path::from(input).with_local(input.name).into(), statements)
            ...
            (Path::from(input).with_local(input.name).into(), Default::default())
        }
    }
}
