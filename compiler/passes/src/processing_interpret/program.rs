...
impl ProgramReconstructor for ProcessingInterpretVisitor<'_> {
    fn reconstruct_program_scope(&mut self, mut input: ProgramScope) -> ProgramScope {
        ...
        if !self.state.is_test {
            for (_i, f) in input.functions.iter() {
                if matches!(f.variant, Variant::Interpret) {
                    self.state.handler.emit_err(TypeCheckerError::interpret_in_non_test(f.identifier, f.span))
                }
            }
        }
        input.functions.retain(|(_i, f)| !matches!(f.variant, Variant::Interpret));
        input
    }
