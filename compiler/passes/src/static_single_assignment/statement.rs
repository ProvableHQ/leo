...
impl StatementConsumer for SsaFormingVisitor<'_> {
    ...
    fn consume_assign(&mut self, mut assign: AssignStatement) -> Self::Output {
        ...
        if let Expression::Path(path) = assign.place {
            ...
            // We really expect `path` to refer to a local variable so we only care about the result of `as_symbol()`.
            let new_place = self.rename_identifier(path.as_identifier());
