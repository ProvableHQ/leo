        ...
        match self.state.type_table.get(&future_variable.id) {
            Some(type_) => {
                if !matches!(type_, Type::Future(_)) {
                    self.emit_err(StaticAnalyzerError::expected_future(
                        future_variable.segments.last().unwrap().name,
                ...
                if self.await_checker.remove(&future_variable.segments.last().unwrap().name) {
