    ...
    fn reconstruct_ternary(&mut self, mut input: TernaryExpression) -> (Expression, Self::AdditionalOutput) {
        ...
        match (if_true, if_false) {
            (Expression::Tuple(tuple_true), Expression::Tuple(tuple_false)) => {
                ...
                let Some(Type::Tuple(tuple_type)) = self.type_table.get(&tuple_true.id()) else {
                ...
                let cond = if let Expression::Identifier(..) = *input.condition {
                    *input.condition
                    ...
                    let place = Identifier::new(self.assigner.unique_symbol("cond", "$$"), self.node_builder.next_id());

                    let definition =
                        self.assigner.simple_definition(place, *input.condition, self.node_builder.next_id());

                    statements.push(definition);

                    self.type_table.insert(place.id(), Type::Boolean);
                    ...
                    izip!(tuple_true.elements, tuple_false.elements, tuple_type.elements()).enumerate()
                {
                    let identifier = Identifier::new(
                        self.assigner.unique_symbol(format_args!("ternary_{i}"), "$$"),
                    ...
                    self.type_table.insert(expression.id(), ty.clone());

                    let definition =
                        self.assigner.simple_definition(identifier, expression, self.node_builder.next_id());
