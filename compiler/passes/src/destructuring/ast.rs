...
impl AstReconstructor for DestructuringVisitor<'_> {
    ...
    fn reconstruct_tuple_access(&mut self, input: TupleAccess) -> (Expression, Self::AdditionalOutput) {
        ...
        match self.tuples.get(&path.as_symbol()).and_then(|tuple_names| tuple_names.get(input.index.value())) {
    ...
    fn reconstruct_assign(&mut self, mut assign: AssignStatement) -> (Statement, Self::AdditionalOutput) {
        ...
        if let Expression::Path(path) = &assign.place {
            if let Type::Tuple(..) = self.state.type_table.get(&value.id()).expect("Expressions should have types.") {
                ...
                let identifiers = self.tuples.get(&path.as_symbol()).expect("Tuple should have been encountered.");

                let Expression::Path(rhs) = value else {
                    panic!("SSA should have ensured this is an identifier.");
                };

                let rhs_identifiers = self.tuples.get(&rhs.as_symbol()).expect("Tuple should have been encountered.");
        ...
        loop {
            ...
            match place {
                Expression::TupleAccess(access) => {
                    ...
                    let tuple_ids = self.tuples.get(&path.as_symbol()).expect("Tuple should have been encountered.");
    ...
    fn reconstruct_definition(&mut self, definition: DefinitionStatement) -> (Statement, Self::AdditionalOutput) {
        ...
        match (definition.place, value, ty) {
            (Single(identifier), Expression::Path(rhs), Type::Tuple(tuple_type)) => {
                ...
                let rhs_identifiers = self.tuples.get(&rhs.as_symbol()).unwrap();
            ...
            (Multiple(identifiers), Expression::Path(rhs), Type::Tuple(..)) => {
                ...
                let rhs_identifiers =
                    self.tuples.get(&rhs.as_symbol()).expect("We should have encountered this tuple by now");
