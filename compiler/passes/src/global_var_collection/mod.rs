...
pub struct GlobalVarCollection;

impl Pass for GlobalVarCollection {
    type Input = ();
    type Output = ();

    const NAME: &'static str = "GlobalVarCollection";

    fn do_pass(_input: Self::Input, state: &mut CompilerState) -> Result<Self::Output> {
        let ast = std::mem::take(&mut state.ast);
        let mut visitor = GlobalVarCollectionVisitor {
            state,
...
struct GlobalVarCollectionVisitor<'a> {
...
impl GlobalVarCollectionVisitor<'_> {
...
impl AstVisitor for GlobalVarCollectionVisitor<'_> {
...
impl ProgramVisitor for GlobalVarCollectionVisitor<'_> {
