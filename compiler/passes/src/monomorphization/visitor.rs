...
impl MonomorphizationVisitor<'_> {
    ...
    pub(crate) fn monomorphize_struct(&mut self, path: &Path, const_arguments: &Vec<Expression>) -> Path {
        ...
        let new_struct_path = path.with_updated_last_symbol(leo_span::Symbol::intern(&format!(
            "{}::[{}]",
            path.as_symbol(),
        ...
        if self.reconstructed_structs.get(new_struct_path.absolute_path()).is_none() {
            ...
            let mut replacer = Replacer::new(replace_path, &self.state.node_builder);
            ...
            struct_.identifier = Identifier::new(new_struct_path.as_symbol(), self.state.node_builder.next_id());
