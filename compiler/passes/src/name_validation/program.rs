...
impl ProgramVisitor for NameValidationVisitor<'_> {
    ...
    fn visit_program_scope(&mut self, input: &ProgramScope) {
        ...
        self.check_name(program_name, "program", &[]);
    ...
    fn visit_struct(&mut self, input: &Composite) {
        ...
        self.check_name(struct_name, item_type, &[]);

        for Member { identifier: member_name, .. } in &input.members {
            if input.is_record {
                self.check_name(*member_name, "record member", &["owner"]);
            } else {
                self.check_name(*member_name, "struct member", &[]);
            }
        }
    }
    ...
    fn visit_function(&mut self, function: &Function) {
        ...
        match function.variant {
            Transition | AsyncTransition => self.check_name(function.identifier, "transition", &[]),
            Inline | Function | AsyncFunction | Script => {}
        }
    }
    ...
    fn visit_function_stub(&mut self, input: &FunctionStub) {
        use Variant::*;
        match input.variant {
            Transition | AsyncTransition => self.check_name(input.identifier, "transition", &[]),
            Inline | Function | AsyncFunction | Script => {}
        }
    }
