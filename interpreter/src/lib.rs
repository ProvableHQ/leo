...
pub fn find_and_run_tests(
    ...
    for (id, function) in interpreter.cursor.functions.clone().into_iter() {
        ...
        // If it's a native test, just store it and move on.
        if let Some(annotation) =
            function.annotations.iter().find(|annotation| annotation.identifier.name == sym::native_test)
        {
            if str_matches() {
        ...
        // If this function is not annotated with interpreted_test, skip it.
        if function.annotations.iter().all(|annotation| annotation.identifier.name != sym::interpreted_test) {
            continue;
        }
