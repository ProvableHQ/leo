        ...
        if let Some(value) = match expression {
            ...
            Expression::AssociatedFunction(function) if step == 1 => {
                ...
                if let CoreFunction::FutureAwait = core_function {
                    ...
                    if value.is_none() {
                        if let CoreFunction::OptionalUnwrap = core_function {
                            halt!(function.span(), "{function}: attempting to unwrap a `None`.");
                        } else {
                            halt!(function.span(), "`{function}` cannot be evaluated.");
                        }
                    }
                    value
                }
