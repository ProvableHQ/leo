    ...
    pub fn assign(&mut self, value: Value, place: &Expression, indices: &mut dyn Iterator<Item = Value>) -> Result<()> {
        ...
        loop {
            match places.last().unwrap() {
                ...
                Expression::Slice(slice) => places.push(&slice.source_array),
    ...
    fn step_expression(&mut self, expression: &Expression, expected_ty: &Option<Type>, step: usize) -> Result<bool> {
        ...
        if let Some(value) = match expression {
            ...
            Expression::Slice(slice) if step == 0 => {
                ...
                if let Some(end) = &slice.stop {
                    push!()(end, &None);
                }
                push!()(&slice.source_array, &None);
                None
            }
            Expression::Slice(slice) if step == 1 => {
                let span = slice.span();
                let (inclusive, end) =
                    if slice.clusivity { (slice.clusivity, Some(self.pop_value()?)) } else { (false, None) };
