...
fn format_root(node: &SyntaxNode, out: &mut Output) {
    if node.children().any(|child| !matches!(child.kind(), IMPORT | PROGRAM_DECL | ANNOTATION)) {
        ...
        match elem {
            ...
            SyntaxElement::Node(n) => {
                ...
                if k.is_type() {
                    ...
                    format_node(&n, out);
                } else if matches!(k, IDENT_PATTERN | TUPLE_PATTERN | WILDCARD_PATTERN) {
                    format_node(&n, out);
                } else if k == ERROR {
...
fn write_node_with_inline_error_verbatim(node: &SyntaxNode, out: &mut Output) -> bool {
    if let Some(next) = node.next_sibling() {
        if should_inline_adjacent_error(&next) {
...
fn should_inline_adjacent_error(node: &SyntaxNode) -> bool {
    let text = node.text().to_string();
    node.kind() == ERROR && node.prev_sibling().is_some() && text.chars().next().is_some_and(|ch| !ch.is_whitespace())
}
