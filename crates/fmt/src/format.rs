...
pub fn format_node(node: &SyntaxNode, out: &mut Output) {
    match node.kind() {
        ...
        // Declarations
        FUNCTION_DEF | FINAL_FN_DEF | QUERY_FN_DEF | CONSTRUCTOR_DEF => format_function(node, out),
...
fn is_block_item(kind: SyntaxKind) -> bool {
    matches!(
        kind,
        FUNCTION_DEF | FINAL_FN_DEF | QUERY_FN_DEF | CONSTRUCTOR_DEF | STRUCT_DEF | RECORD_DEF | INTERFACE_DEF
    )
}
...
fn format_function(node: &SyntaxNode, out: &mut Output) {
    ...
    // identifier `query` as its first IDENT child (followed by `fn` and then
    // the real function-name IDENT). We need to print that first IDENT with a
    // trailing space so the output reads `query fn name(...)`.
    let is_query_fn = node.kind() == QUERY_FN_DEF;
        ...
        match elem {
            SyntaxElement::Token(tok) => {
                ...
                match k {
                    ...
                    IDENT if is_query_fn && !first_ident_seen => {
                        // Contextual `query` keyword.
                        out.write(tok.text());
                        out.space();
