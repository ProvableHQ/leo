...
impl<'a> ConversionContext<'a> {
    ...
    /// token. Used for `view fn name(...)` — the VIEW_FN_DEF node has the
    /// contextual `view` IDENT as its first IDENT child, followed by `fn`,
    ...
    fn to_function(&self, node: &SyntaxNode, is_in_program_block: bool) -> Result<leo_ast::Function> {
        ...
        // identifier `view` as its first IDENT child, followed by `fn` and then the
        // real function name. Skip past the leading `view` token so we extract the
        // correct identifier.
        let identifier = if node.kind() == VIEW_FN_DEF {
    ...
    fn to_function_prototype(&self, node: &SyntaxNode) -> Result<leo_ast::FunctionPrototype> {
        ...
        // Detect a leading contextual `view` IDENT (interface-position view fn prototype).
        // When present, the function name is the first IDENT *after* KW_FN.
        let is_view = tokens(node)
            .find(|t| t.kind() == IDENT || t.kind() == KW_FN)
            .map(|t| t.kind() == IDENT && t.text() == "view")
            .unwrap_or(false);
        let variant = if is_view { leo_ast::Variant::View } else { leo_ast::Variant::Fn };
        let identifier = if is_view {
            self.require_ident_after_kw_fn(node, "function name")
        } else {
            self.require_ident(node, "function name")
        };
