...
impl Pass for TypeChecking {
    ...
    fn do_pass(input: Self::Input, state: &mut CompilerState) -> Result<Self::Output> {
        ...
        // Exported structs are part of a program's public type interface
        visitor.used_composites.extend(
            visitor
                .state
                .symbol_table
                .iter_structs()
                .filter(|(loc, s)| {
                    s.is_exported == Some(true)
                        && s.const_parameters.is_empty()
                        && !visitor.state.symbol_table.is_library(loc.program)
                })
                .map(|(loc, _)| loc.clone()),
        );
