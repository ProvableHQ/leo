...
impl fmt::Display for ProgramScope {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        for (_, function) in self.functions.iter().filter(|f| f.1.variant.is_query()) {
            writeln!(f, "{}", Indent(function))?;
        }
        writeln!(f, "}}")?;

        for (_, function) in self.functions.iter().filter(|f| !f.1.variant.is_entry() && !f.1.variant.is_query()) {
            writeln!(f, "{}", Indent(function))?;
        }
        Ok(())
    }
