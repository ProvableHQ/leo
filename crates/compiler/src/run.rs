...
pub struct Case {
    ...
    /// Each entry is `(mapping_name, key_str, value_str)` and applies to `program_name`.
    /// Useful for query test cases — `run_without_ledger` doesn't run finalize blocks,
    /// so mappings are otherwise empty.
    pub seed_mapping: Vec<(String, String, String)>,
}
...
pub fn run_without_ledger(config: &Config, cases: &[Case]) -> Result<Vec<EvaluationOutcome>> {
    ...
    let outcomes: Vec<EvaluationOutcome> = cases
        ...
        .map(|case| {
            ...
            // the finalize store. Queries that read mappings need the mappings to be present in
            ...
            // query test cases where `run_without_ledger` doesn't run finalize blocks, so
            // mappings are otherwise empty.
            for (mapping_name_str, key_str, value_str) in &case.seed_mapping {
            ...
            // Dispatch on whether the named function is a `query`. Queries don't go through
            // authorize/evaluate; they go through `Process::evaluate_query_at_height`, which
            // returns plaintext outputs directly without producing a transition.
            let is_query = vm
                .process()
                .get_stack(program_id)
                .map(|stack| stack.program().contains_query(&function_id))
                .unwrap_or(false);
            ...
            if is_query {
                ...
                {
                    Ok(v) => v,
                    Err(e) => return failed_outcome(format!("Failed to parse query input: {e}")),
                };
                // For an empty in-memory consensus store there is no block 0, so route directly
                // through `Process::evaluate_query_at_height` with a fabricated
                // `FinalizeGlobalState`. Tests are off-consensus by construction, so the
                // values here only matter for queries that read `block.height` / `network.id`.
                let state = match snarkvm::synthesizer::program::FinalizeGlobalState::new::<CurrentNetwork>(
                    ...
                    Ok(s) => s,
                    Err(e) => return failed_outcome(format!("Failed to build FinalizeGlobalState: {e}")),
                };
                let response = match catch_unwind(AssertUnwindSafe(|| {
                    vm.process().evaluate_query_at_height(
                        state,
                        vm.finalize_store(),
                        program_id,
                        function_id,
            ...
            let response =
                match catch_unwind(AssertUnwindSafe(|| vm.process().evaluate::<AleoTestnetV0>(authorization))) {
                    Ok(Ok(resp)) => resp,
                    Ok(Err(e)) => return failed_outcome(format!("{e}")),
                    Err(e) => return failed_outcome(format!("{e:?}")),
                };
