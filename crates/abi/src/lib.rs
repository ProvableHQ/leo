...
pub fn generate(ast: &ast::Program) -> abi::Program {
    ...
    let queries =
        scope.functions.iter().filter(|(_, f)| f.variant.is_query()).map(|(_, f)| convert_function(f, &ctx)).collect();

    let implements: Vec<abi::InterfaceRef> =
        scope.parents.iter().filter_map(|(_, ty)| interface_ref_from_type(ty, &program)).collect();

    let mut program =
        abi::Program { program, implements, structs, records, mappings, storage_variables, functions, queries };
...
pub fn prune_non_interface_types(program: &mut abi::Program) {
    ...
    for function in program.functions.iter().chain(program.queries.iter()) {
