...
//! `leo build` clap surface. The actual build logic lives in
//! [`leo_cli_core::commands::build::handle_build`]; this file just collects
//! the `--option` flags, resolves the network/endpoint, and forwards.
...
/// Resolve env defaults and forward to `leo_cli_core`. Kept thin so all the
/// real work lives in one place that `leo-wasm` can also call into.
fn run_build(command: &LeoBuild, context: Context) -> Result<Package> {
    ...
    leo_cli_core::commands::build::handle_build(
        &command.options,
        network,
        &endpoint,
