...
create_messages!(
    ...
    imported_program_not_found {
        ...
        msg: format!("`{main_program_name}` imports `{dependency_name}`, but `{dependency_name}` is not found in program manifest. Use `leo add --help` for more information on how to add a dependency."),
        help: None,
    }
