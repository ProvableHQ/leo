...
use crate::{load_asg_imports, load_asg, mocked_resolver};
...
fn test_name() {
    let mut imports = mocked_resolver();
    imports.packages.insert("hello-world".to_string(), load_asg(include_str!("src/hello-world.leo")).unwrap());
    imports.packages.insert("a0-f".to_string(), load_asg(include_str!("src/a0-f.leo")).unwrap());
...
fn test_many_import() {
    let mut imports = mocked_resolver();
    imports.packages.insert("test-import".to_string(), load_asg(include_str!("src/test-import.leo")).unwrap());
    imports.packages.insert("bar".to_string(), load_asg(include_str!("imports/bar/src/lib.leo")).unwrap());
    imports.packages.insert("bar.baz".to_string(), load_asg(include_str!("imports/bar/src/baz.leo")).unwrap());
    imports.packages.insert("bar.baz".to_string(), load_asg(include_str!("imports/bar/src/baz.leo")).unwrap());
    imports.packages.insert("bar.bat.bat".to_string(), load_asg(include_str!("imports/bar/src/bat/bat.leo")).unwrap());
