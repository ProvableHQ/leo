use crate::Span;
use crate::{ Statement, Expression, BlockStatement, FromAst, Scope, AsgConvertError, Type };
...
impl FromAst<leo_ast::ConditionalStatement> for ConditionalStatement {
    fn from_ast(scope: &Scope, statement: &leo_ast::ConditionalStatement, _expected_type: Option<Type>) -> Result<Self, AsgConvertError> {
        let condition = Arc::<Expression>::from_ast(scope, &statement.condition, Some(Type::Boolean))?;
