...
create_messages!(
    ...
    @formatted
    input_cannot_be_optional {
        args: (variant: impl Display, name: impl Display),
        msg: format!(
            "`{variant} {name}` cannot have an input of optional type or a type that contains an optional",
        ),
        help: None,
    }

    @formatted
    output_cannot_be_optional {
        args: (variant: impl Display, name: impl Display),
        msg: format!(
            "`{variant} {name}` cannot return an optional type or a type that contains an optional",
        ),
        help: None,
    }
