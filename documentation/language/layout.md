...
- A single, mandatory [`constructor`](./structure.md#constructor)
- [Mappings](./structure.md#mappings) and [Storage](./structure.md#storage)
- [Records](./structure.md#record)
...
- [Imports](#imports)
- [Constants](./structure.md#constant)
- [Structs](./structure.md#struct)
...
A downward path from parent to child always works, for example from `outer.leo` to `outer/inner.leo`. However, upward and sideways paths from a submodule do not work. In `outer/inner.leo`, `common::foo` resolves to `outer::inner::common::foo`. Leo does not have syntax to reference an item outside the current module subtree.
...
Leo puts helper `fn` code from `program.aleo::submodule::name(...)` directly in the caller bytecode. These helpers are not separate on-chain calls and do not appear in the provider ABI. Only top-level entry functions in `program provider.aleo { ... }` remain in its on-chain interface.
