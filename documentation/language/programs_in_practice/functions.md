...
- **On-chain reads and proof verification.** A view can use `std::ctx::block_timestamp()`, `std::ctx::program_owner()`, `Snark::verify`, and `Snark::verify_batch`. These operations do not change state.
