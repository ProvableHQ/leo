...
An `interface` declaration specifies the functions, records, mappings, and storage variables that a program must provide. Interfaces are compile-time contracts and do not change the generated bytecode. The compiler verifies that an implementing program provides all required members. Interfaces also enable dynamic calls. The caller knows the available operations but can select the program at runtime. You can declare an interface outside the `program {}` block, in a submodule, or in a library package.
...
The compiler **flattens** all members when a program implements multiple interfaces with `+`. It also flattens members when an interface inherits from another interface with `:`. Then, the compiler checks each member for conflicts. Two interfaces with the same member name must define the same shape. Otherwise, compilation reports a `conflicting interface member` error.
...
- **Records** with the same name require the child prototype to list each field in the parent prototype. Each field must have the same type and visibility mode (`public`, `private`, or `constant`). A missing parent field causes a `conflicting_record_field` error. The trailing `..` lets an implementing program add fields. It does not let a child interface omit required fields.
...
:::note
Dynamic mapping reads are a type-checked alternative to the [`_dynamic_get`, `_dynamic_contains`, and `_dynamic_get_or_use`](./intrinsics.md) intrinsics. The interface form verifies that the named mapping exists. It also verifies the key, value, and default types. The intrinsics accept arbitrary runtime identifiers, so the caller must do these checks. Use the interface form when an interface is available.
:::
...
`dyn record` complements dynamic calls. A dynamic call can route logic to any compliant program. A `dyn record` lets the caller accept, inspect, and forward records from programs not known at compile time. The type system continues to apply its safety checks.
...
A common pattern receives a `dyn record` as a function parameter. Read the required fields with type annotations. Then, send the record to a dynamic call or return it from the function.
...
The call halts if the runtime field type does not match the declared type. For example, this occurs when `r.balance` is a `field`, but the binding declares a `u64`.
...
- **`dyn record` → static record (explicit)**: **not supported.** The compiler rejects `r as Token`. The runtime cannot verify that the actual layout of `r` matches the declared layout of `Token`. Access each field separately with a type annotation. See [Reading Fields](#reading-fields).

- **`dyn record` → static record (implicit)**: occurs when a dynamic-call interface declares a concrete record parameter. See [Case D](#dynamic-records-and-dynamic-calls). The call narrows the record implicitly. The call halts if the actual record layout does not match.
...
A dynamic call treats all record arguments as `dyn record`. It also returns all record values as `dyn record`. This behavior also applies when the interface uses a concrete static record type. The interface declaration and caller input produce four cases:
