...
```leo file=../code_snippets/data_types.leo#addresses
```
...
```leo file=../code_snippets/data_types.leo#booleans
```
...
```leo file=../code_snippets/data_types.leo#int_basic
```

Underscores `_` can be used to separate digits in integer literals.

```leo file=../code_snippets/data_types.leo#int_underscore
```
...
```leo file=../code_snippets/data_types.leo#int_cast
```
...
```leo file=../code_snippets/data_types.leo#field_literals
```
...
```leo file=../code_snippets/data_types.leo#group_literals
```

The aforementioned generator point can be obtained via a constant associated to the `group` type.

```leo file=../code_snippets/data_types.leo#group_gen
```
...
```leo file=../code_snippets/data_types.leo#scalar_literals
```
...
```leo file=../code_snippets/data_types/signatures.leo#file
```

Signature literals can be also be used. For example:

```leo file=../code_snippets/data_types.leo#signature_literal
```
...
```leo file=../code_snippets/data_types.leo#identifier_literal
```
...
```leo file=../code_snippets/data_types.leo#tuple_basic
```
...
```leo file=../code_snippets/data_types.leo#tuple_access_fn
```
...
```leo file=../code_snippets/data_types/struct_s.leo#file
```

Structs defined in external programs can be referenced and constructed using their fully qualified name:

```leo file=../code_snippets/data_types/external_consumer.leo#file
```

Leo supports **const generics** for struct types:

```leo file=../code_snippets/data_types/matrix.leo#file
```
...
```leo file=../code_snippets/data_types.leo#option_basic
```
...
```leo file=../code_snippets/data_types.leo#option_unwrap
```

Option types can also be stored in arrays and structs:

```leo file=../code_snippets/data_types/options.leo#file
```
...
```leo file=../code_snippets/data_types.leo#type_inference
```
...
```leo file=../code_snippets/data_types/type_inference.leo#file
```
