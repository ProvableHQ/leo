...
On invoking the build command, Leo automatically creates a `build/⁠` and `outputs/`⁠ folder in the project directory. Inside `build/`, every program - your own program and each dependency - gets its own `build/{program}/` directory containing its compiled `.aleo` bytecode and ABI. The `outputs` directory is used to store intermediate artifacts from compilation.
