...
See the [Upgradability Guide](../guides/program_upgradability.md) for more details on how to make your program upgradable.

## Workspace Deployment

When your project uses a [workspace](./workspaces.md), `leo deploy` from the workspace root deploys all members in dependency order. Leo uses a single shared VM across all members, which provides accurate fee estimation and ensures programs shared between members are deployed exactly once.
